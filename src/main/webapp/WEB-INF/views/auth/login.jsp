...
body::before{
    ...
    background:radial-gradient(circle,rgba(78,115,223,0.12) 0%,transparent 65%);
    top:-200px;left:-200px;pointer-events:none;
}
body::after{
    content:'';position:fixed;
    width:500px;height:500px;border-radius:50%;
    background:radial-gradient(circle,rgba(28,200,138,0.08) 0%,transparent 65%);
    bottom:-150px;right:-150px;pointer-events:none;
}
.card{
    width:100%;max-width:920px;
    display:grid;grid-template-columns:1fr 1fr;
    min-height:580px;border-radius:22px;
...
.left{
    background:linear-gradient(155deg,#0f3460 0%,#16213e 50%,#12122a 100%);
    padding:50px 44px;
...
.rtab:hover{color:rgba(255,255,255,.6)}
...
.finput.tf:focus{border-color:rgba(28,200,138,.55);background:rgba(28,200,138,.06);box-shadow:0 0 0 4px rgba(28,200,138,.1)}
.finput.af:focus{border-color:rgba(231,74,59,.55);background:rgba(231,74,59,.06);box-shadow:0 0 0 4px rgba(231,74,59,.1)}
.eyebtn{position:absolute;right:9px;background:none;border:none;color:rgba(255,255,255,.3);cursor:pointer;padding:5px 7px;border-radius:6px;font-size:.82rem;transition:all .2s;z-index:2}
.eyebtn:hover{color:white;background:rgba(255,255,255,.07)}
.optrow{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px}
.chk{display:flex;align-items:center;gap:7px;font-size:.8rem;color:rgba(255,255,255,.45);cursor:pointer}
.chk input{accent-color:#4e73df;cursor:pointer}
...
.btnl:active{transform:scale(.98)}
.bl-s{background:linear-gradient(135deg,#4e73df,#224abe);color:white;box-shadow:0 6px 22px rgba(78,115,223,.45)}
.bl-s:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(78,115,223,.6)}
.bl-t{background:linear-gradient(135deg,#1cc88a,#13a66e);color:white;box-shadow:0 6px 22px rgba(28,200,138,.45)}
.bl-t:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(28,200,138,.6)}
.bl-a{background:linear-gradient(135deg,#e74a3b,#b83228);color:white;box-shadow:0 6px 22px rgba(231,74,59,.45)}
.bl-a:hover{transform:translateY(-2px);box-shadow:0 10px 30px rgba(231,74,59,.6)}
...
.dl i{font-size:.85rem}
.dis i{color:#7b9ef0}.dit i{color:#4dffc5}.dia i{color:#ff8a80}
.dl strong{color:rgba(255,255,255,.75)}
.da{font-size:.68rem;font-weight:700;color:#4e73df;opacity:0;transition:opacity .15s}
.dr:hover .da{opacity:1}
...
<div class="card">
    ...
    <div class="left">
        <div>
            ...
            <div class="lc">
                ...
                <p class="ld">
                    India ke top instructors se seekho.
                    Real projects, certificates aur
                    lifetime access ke saath career banao.
                </p>
                <div class="fi">
                    <div class="fd fb">
        ...
        <div class="stats">
            <div>
                <div class="sn">1K+</div>
                <div class="sl">Courses</div>
            </div>
            <div>
                <div class="sn">50K+</div>
                <div class="sl">Students</div>
            </div>
            <div>
                <div class="sn">200+</div>
                <div class="sl">Teachers</div>
            </div>
            <div>
                <div class="sn" style="color:#f6c23e">4.9★</div>
    ...
    <div class="right">
        ...
        <div class="ri">
            ...
            <div class="t2">Role select karke login karein</div>
            ...
            <c:if test="${param.error != null}">
                <div class="ae">
                    <i class="fa-solid fa-circle-xmark"></i>
                    <span>Email ya password galat hai!</span>
                </div>
            </c:if>

            <!-- Role Tabs -->
            <div class="rtabs">
                <button class="rtab on-s" id="rb-s"
                        onclick="pick('s')">
                    <i class="fa-solid fa-user-graduate"></i>
                    <span>Student</span>
                </button>
                <button class="rtab" id="rb-t"
                        onclick="pick('t')">
                    <i class="fa-solid fa-chalkboard-user"></i>
                    <span>Teacher</span>
                </button>
                <button class="rtab" id="rb-a"
                        onclick="pick('a')">
            ...
            <div class="strip ss" id="strip">
                ...
                <span id="stxt">
                    Student portal — Courses access karein
                </span>
            </div>
            ...
            <form action="${pageContext.request.contextPath}/perform_login"
                ...
                <button type="submit" class="btnl bl-s"
                        id="lbtn">
            ...
            <div class="demo">
                ...
                <div class="dr dis"
                     onclick="fill('student@elearn.com','Student@123','s')">
                    <div class="dl">
                        <i class="fa-solid fa-user-graduate"></i>
                        <span>
                    ...
                    <span class="da">Fill →</span>
                </div>
                <div class="dr dit"
                     onclick="fill('teacher@elearn.com','Teacher@123','t')">
                    <div class="dl">
                        <i class="fa-solid fa-chalkboard-user"></i>
                        <span>
                ...
                <div class="dr dia"
                     onclick="fill('admin@elearn.com','Admin@123','a')">
                    <div class="dl">
                        <i class="fa-solid fa-shield-halved"></i>
                        <span>
                            <strong>Admin</strong>
...
<script>
var R={
    s:{on:'on-s',st:'ss',txt:'Student portal \u2014 Courses access karein',btn:'bl-s',lbl:'Sign in as Student',ico:'fa-user-graduate'},
    t:{on:'on-t',st:'st',txt:'Teacher portal \u2014 Courses manage karein',btn:'bl-t',lbl:'Sign in as Teacher',ico:'fa-chalkboard-user'},
    a:{on:'on-a',st:'sa',txt:'Admin portal \u2014 Full platform control',btn:'bl-a',lbl:'Sign in as Admin',ico:'fa-shield-halved'}
};
var cur='s';
function pick(r){
    cur=r;var c=R[r];
    ...
    var fc=r==='t'?'tf':r==='a'?'af':'';
    ['eml','pwd'].forEach(function(id){
        document.getElementById(id).className='finput '+fc;
    });
}
function toggleEye(){
...
function fill(email,pwd,role){
    ...
    ['eml','pwd'].forEach(function(id){
        ...
        el.style.borderColor='#1cc88a';
        el.style.boxShadow='0 0 0 4px rgba(28,200,138,0.15)';
